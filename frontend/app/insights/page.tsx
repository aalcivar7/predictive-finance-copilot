...
const cfg = {
  warning: { emoji: '⚠️', label: 'Alert',     bg: '#241414', border: '#4a2020', labelColor: '#f87171', labelBg: 'rgba(244,63,94,0.1)' },
  success: { emoji: '✅', label: 'Strength',   bg: '#0f2420', border: '#1a4035', labelColor: '#34d399', labelBg: 'rgba(16,185,129,0.1)' },
  tip:     { emoji: '💡', label: 'Tip',        bg: '#1e1430', border: '#2d1f55', labelColor: '#a78bfa', labelBg: 'rgba(124,58,237,0.12)' },
};
...
export default function InsightsPage() {
  ...
  useEffect(() => {
    getInsights().then((d) => setInsights(d.insights)).finally(() => setLoading(false));
  }, []);

  const sections = [
    { key: 'warning', label: 'Alerts' },
    { key: 'success', label: 'Strengths' },
    { key: 'tip',     label: 'Recommendations' },
  ];
    ...
    <ProtectedRoute>
      <div style={{ maxWidth: '720px', margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: '24px' }}>
          <h1 style={{ fontSize: '20px', fontWeight: 700, color: '#f0f0f5' }}>Smart Insights</h1>
          <p style={{ fontSize: '13px', color: '#60607a', marginTop: '2px' }}>Personalized analysis based on your profile</p>
        </div>
        ...
        {!loading && insights.length === 0 && (
          <div className="card" style={{ textAlign: 'center', padding: '60px' }}>
            ...
            <p style={{ color: '#c0c0d0', fontWeight: 600, marginBottom: '6px' }}>No insights yet</p>
            <p style={{ color: '#50505e', fontSize: '13px' }}>Update your financial profile on the Dashboard first.</p>
          </div>
        )}
